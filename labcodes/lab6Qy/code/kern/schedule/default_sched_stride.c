...
#define BIG_STRIDE 0x7FFFFFFF  // 2^31-1, avoid overflow
...
stride_pick_next(struct run_queue *rq)
{
     ...
     if (!list_empty(&(rq->run_list))) {
          ...
          list_entry_t *tmp;
          for (tmp = list_next(le); tmp != &(rq->run_list); tmp = list_next(tmp)) {
               struct proc_struct *q = le2proc(tmp, run_link);
               if (q->lab6_stride < min_stride) {
                    min_stride = q->lab6_stride;
                    p = q;
               }
          }
          if (p->lab6_priority != 0) {
